...
export async function preEdit({filePath}) {
  ...
  if (filePath.match(/\.(ts|tsx|js|jsx)$/)) {
    ...
    try {
      ...
      console.log("Warning: File needs formatting - will format after edit");
    }
  }
...
export async function postEdit({filePath, success}) {
  ...
  if (filePath.match(/\.(ts|tsx|js|jsx)$/)) {
    try {
      ...
      console.log("Warning: Lint errors detected - please review");
    }
  }
  ...
  if (filePath.match(/\.(ts|tsx)$/)) {
    try {
      ...
      console.log("Warning: TypeScript errors detected - please review");
    }
  }
}
