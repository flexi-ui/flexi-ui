...
import type { LinkVariantProps } from '@flexi-ui/theme'
import type { MouseEventHandler } from 'react'

import { link } from '@flexi-ui/theme'
import { useAriaLink } from '@flexi-ui/use-aria-link'
import {
  HTMLFlexiUIProps,
  mapPropsVariants,
  PropGetter,
...
import { dataAttr, objectToDeps } from '@flexi-ui/shared-utils'
import { ReactRef } from '@flexi-ui/react-utils'
import { useMemo, useCallback } from 'react'
import { mergeProps } from '@react-aria/utils'

interface Props extends HTMLFlexiUIProps<'a'>, LinkVariantProps {
  /**
   * Ref to the DOM node.
   */
...
export function useLink(originalProps: UseLinkProps) {
  ...
  // use `@flexi-ui/use-aria-link` to suppress onClick deprecation warning
